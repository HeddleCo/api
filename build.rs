...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for package in ["v1alpha1", "v2alpha1"] {
        ...
        for entry in fs::read_dir(package_root)? {
            let path = entry?.path();
            if path.extension().is_some_and(|extension| extension == "proto") {
                protos.push(path);
            }
        }
    }
    ...
    method_generation::write(&descriptor, &output.join("heddle_api_methods.rs"), "heddle.api.v1alpha1")?;
    method_generation::write(&descriptor, &output.join("heddle_api_v2_methods.rs"), "heddle.api.v2alpha1")?;
