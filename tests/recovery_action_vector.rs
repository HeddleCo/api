...
fn rust_recovery_action_vector_for_browser_parity() {
    ...
    let reference = RecordRef { id: "123e4567-e89b-12d3-a456-426614174002".into(), spool: None };
    let version: Vec<u8> = (0u8..32).collect();
    let key: Vec<u8> = (32u8..64).collect();
    let canonical = identity_management::recovery_action(account, operation, Some(&reference), &version, &key)
        .expect("fixed unscoped recovery action vector");
    let signing = identity_management::signing_bytes(identity_management::RECOVERY_POSSESSION, &canonical)
    ...
    assert_eq!(signing.len(), identity_management::RECOVERY_POSSESSION.len() + 1 + canonical.len());
}
