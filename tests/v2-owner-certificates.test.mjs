...
import { canonicalMintRootAttachment, mintRootAttachmentSigningDigest, signOwnerMintRootAttachment, verifyOwnerMintRootAttachment } from '../packages/typescript/dist/v1alpha2/owner-certificates.js';
...
const signed = fromBinary(SignedOwnerMintRootAttachmentSchema, hex(fixture.record_hex));
...
test('mint-root certificate matches independent Rust canonical bytes, digest, signature and protobuf container', async () => {
  ...
  await verifyOwnerMintRootAttachment(signed, expected);
  const actual = await signOwnerMintRootAttachment(signed.attachment, signer);
  assert.deepEqual(actual.ownerSignature.signature, hex(fixture.signature_hex));
  assert.deepEqual(toBinary(SignedOwnerMintRootAttachmentSchema, actual), hex(fixture.record_hex));
});
test('certificate cannot select its own trusted account, owner state, mint root or lifetime', async () => {
  for (const field of ['accountUuid', 'ownerStateHash', 'ownerPublicKey', 'mintRootPublicKey']) {
    ...
    await assert.rejects(verifyOwnerMintRootAttachment(signed, altered), /independently verified/);
  }
  await assert.rejects(verifyOwnerMintRootAttachment(signed, { ...expected, ownerSequence: 1n }), /independently verified/);
  await assert.rejects(verifyOwnerMintRootAttachment(signed, { ...expected, nowUnixSeconds: 999998n }), /currently valid/);
  await assert.rejects(verifyOwnerMintRootAttachment(signed, { ...expected, nowUnixSeconds: 1000050n }), /currently valid/);
});
test('owner signature and signer identity are verified, including a signer returning invalid bytes', async () => {
  ...
  await assert.rejects(verifyOwnerMintRootAttachment(altered, expected), /owner signature/);
  const wrongId = structuredClone(signed); wrongId.ownerSignature.signerKeyId[0] ^= 1;
  await assert.rejects(verifyOwnerMintRootAttachment(wrongId, expected), /signer identity/);
  await assert.rejects(signOwnerMintRootAttachment(signed.attachment, { ...signer, sign: async () => new Uint8Array(64) }), /owner signature/);
});
test('malformed version, keys, UUID and integer intervals fail before signing', async () => {
  ...
  for (const change of [{ formatVersion: 2 }, { accountUuid: new Uint8Array(16) }, { nonce: new Uint8Array(31) }, { ownerSequence: 1n << 64n }, { expiresAtUnixSeconds: -1n }, { ownerKey: { algorithm: 0, publicKey: expected.ownerPublicKey } }]) {
    await assert.rejects(signOwnerMintRootAttachment({ ...signed.attachment, ...change }, count));
  }
