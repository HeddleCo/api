...
fn remote_sync_destination_is_an_explicit_thread_identity() {
    ...
    let remote = pool
        .get_message_by_name("heddle.api.v1alpha2.RemoteLinkRecord")
    ...
    assert_eq!(thread.full_name(), "heddle.api.v1alpha2.ThreadRef");
    assert!(
        !target.is_list(),
...
fn remote_policy_deadline_and_atomic_repository_replacement_are_typed() {
    ...
    for (message, field, number, target) in [
        ...
        (
            ...
            "replace_section",
            7,
            "heddle.api.v1alpha2.SectionReplacement",
        ),
    ] {
        let message = pool
