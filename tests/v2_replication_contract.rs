...
fn bulk_publication_has_a_thread_bound_upload_and_a_durable_availability_receipt() {
    ...
    for field in [
        ...
        assert!(
            open.get_field_by_name(field).is_some(),
            "opening binds {field}"
        );
    }
    assert!(
        open.get_field_by_name("expected_tip").is_none(),
        "content availability must not replace concurrent heads"
    );
    let client = pool.get_message_by_name(rpc.input).expect("upload frames");
    assert!(
        client.get_field_by_name("pack").is_some(),
        "client can stream pack bytes"
    );
    let server = pool
        .get_message_by_name(rpc.output)
        .expect("response frames");
    let receipt = server
        .get_field_by_name("receipt")
        .expect("durable content receipt");
    assert_eq!(
        receipt
            .kind()
            .as_message()
