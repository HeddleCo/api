...
fn metadata_and_decoded_mutation_cannot_select_different_operation_ids() {
    ...
    assert_eq!(
        method.client_operation_id(&valid).expect("valid metadata"),
        Some("first")
    );
    let mut ambiguous = valid.clone();
    ambiguous.extend(request("second"));
    assert_eq!(
        RenameThreadRequest::decode(ambiguous.as_slice())
            .expect("valid protobuf merge")
            .client_operation_id,
        "second"
    );
    assert!(
        method.client_operation_id(&ambiguous).is_err(),
        "reject ambiguous mutation identity before dispatch"
    );
    let mut duplicate = valid.clone();
    duplicate.extend(valid);
    assert!(
        method.client_operation_id(&duplicate).is_err(),
        "identical duplicate fields are ambiguous too"
    );
}
...
fn metadata_validates_the_whole_envelope_and_operation_id_wire_type() {
    ...
    for suffix in [&[0xff][..], &[0x00], &[0x08, 0x01], &[0x12, 0xff]] {
        ...
        assert!(
            method.client_operation_id(&malformed).is_err(),
            "invalid tail {suffix:?} must not be hidden by an earlier operation ID"
        );
    }
    assert!(
        method.client_operation_id(&[0x08, 0x01]).is_err(),
        "operation ID must be length-delimited"
    );
    assert_eq!(
        method.client_operation_id(&[]).expect("empty message"),
        None
    );
    ...
    assert_eq!(
        method
            .client_operation_id(&with_unknown)
